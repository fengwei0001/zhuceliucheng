...
## 六、广播内容设计：JD 完整公开，简历脱敏保护

广播是整个匹配系统的信息载体。每条广播有两个内容字段：
- **`content`**：自然语言描述（Agent 根据本地文件生成）
- **`notes_json`**：结构化标注（type / category / tags / summary / keywords 等，Agent 端生成，服务器透传）

### 6.1 核心原则：两侧信息公开程度不对称
...
```json
{
  ...
  "keywords": ["推荐系统", "PyTorch", "特征工程", "A/B实验"],
  "expected_response": {
    "what": "技术背景、相关项目经验、可实习时间",
    "constraints": "2027届及以后，每周至少4天，至少3个月",
    "deadline": "2026-07-01",
    "example": "我是XX大学CS硕士，做过XX推荐项目，7月可到岗，每周5天"
  },
...
```json
{
  ...
  "keywords": ["推荐系统", "RAG", "PyTorch", "LangChain", "特征工程"],
  "source_type": "original"
}
```
